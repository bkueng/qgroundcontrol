    ...
    _compid(componentId)
{
    auto error_cb = [componentId](int num_events_lost) {
        qCWarning(Events) << "Events got lost:" << num_events_lost << "comp_id:" << componentId;
    };
...
void EventHandler::gotEvent(const mavlink_event_t& event)
{
    if (!_parser.hasDefinitions()) {
        ...
        qCDebug(Events) << "No metadata, queuing event, ID:" << event.id << "num pending:" << _pendingEvents.size();
